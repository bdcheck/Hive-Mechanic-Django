...
class Integration(models.Model):
    ...
    def process_incoming(self, payload):
        if self.type == 'twilio':
            from twilio_support.models import process_incoming
            
            process_incoming(payload)
        else:
            raise Exception('No "' + self.type + '" method implemented to process payload: ' + json.dumps(payload, indent=2))

    def process_player_incoming(player_lookup_key, player_lookup_value, payload):
        player_match = None
        ...
        if player_match is not None:
            ...
            session.process_incoming(integation, payload)
    
    def execute_actions(self, session, actions):
